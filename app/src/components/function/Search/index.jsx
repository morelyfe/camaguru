		...
		if(search.keyword !== '') {
			...
			.then(res => {
				...
				if(res.data.length === 0) {
					confirmAlert({
						message: 'Matching data does not exist!',
						buttons: [
							{
								label: 'Okay'
							}
		...
		<div className='search'>
			<div className='inner-container'>
				<form name='search' onSubmit={_handleForm} autoComplete='off'>
					...
					{ search.users.length === 0 && search.posts.length === 0 ? <p className='search-info'>Press enter to search for users or posts</p> : ''}
				</form>
				{ search.type === 0 ? search.users.map((user, index) => 
					<div className='search-user-container' key={index} onClick={ () => _handleProfilePage(user.id) }>
						<div className='search-user-profile'  style={
							...
							:
							{ backgroundImage: 'url(\'/images/' + user.picture + '\')' }
						}></div>
						<div className='search-user-nickname'>@{user.username}</div>
				...
				<div className='search-post-container'>
					{ search.type === 1 ? search.posts.map((post, index) => 
						<div className={isMobile ? 'search-post search-post-mobile' : 'search-post'} onClick={ () => _handleDetail(post.id) } key={index}>
							<img className='search-picture' src={'/images/' + post.picture} onLoad={ () => document.getElementById('search-load-' + post.id).style.display = 'none' } alt='post' />
