...
const Post = (props) => {
	...
	const _handleComments = () => {
		...
		if(document.getElementById('post-comment-box-' + props.data.id).value === '') {
			confirmAlert({
				message: 'Please write a comment first',
				buttons: [
					{
						label: 'Okay'
					}
		...
		_handleTextareaSize();
		dispatch(post_posts(posts));
		axios.post('/comments/insert', {
			token: auth.token,
			user_id: user.user.id,
			user_username: user.user.nickname,
			post_id: props.data.id,
			content: value,
		})
		.then(res => {
	...
	const _handleDeletePost = () => {
		if(props.data.user_id === user.user.id) {
			...
			confirmAlert({
				message: 'This feature is only available for its own creator',
				buttons: [
					{
						label: 'Okay'
					}
	...
	// 	const url = 'https://camagru.com:8443/share?sid=' + props.data.id;
	...
	return (
		<div className='post'>
			<div className='post-profile' style={
				...
				:
				{ backgroundImage: 'url(\'/images/' + props.data.user_picture + '\')' }
			...
			<img className='post-picture' src={'/images/' + props.data.picture} onClick={ () => _handleDetail() } onLoad={ () => document.getElementById('post-load-' + props.data.id).style.display = 'none' } alt='post' />
